...
const saveCategory = async () => {
  ...
  // Create a deep copy to remove reactivity and ensure clean object
  const currentSkills = JSON.parse(JSON.stringify(skills.value || {}));
  // Firebase removes empty arrays, so we save a placeholder object to keep the category
...
const saveTool = async () => {
  ...
  // Create a deep copy to remove reactivity
  const newSkills = JSON.parse(JSON.stringify(skills.value || {}));
  // Handle case where category might be the placeholder object or undefined
  const currentCategoryData = newSkills[category];
  const tools = Array.isArray(currentCategoryData) ? [...currentCategoryData] : [];
...
const deleteTool = async (category: string, toolName: string) => {
  if (confirm(`Excluir a ferramenta "${toolName}" da categoria "${category}"?`)) {
    ...
    if (Array.isArray(currentCategoryData)) {
      ...
      // If tools array is empty, save placeholder to prevent category deletion by Firebase
      if (tools.length === 0) {
